...
// Author: Iram Naseer
// Description: Terminal-based Tic Tac Toe game where the AI plays optimally.
// Language: C++
// Date: October 2025

// Headers and Using Declarations(prevvent rewriting std::)
#include <iostream>
#include <algorithm>
#include <vector>
...
using std::cin;
using std::endl;
//WIN = +2000, LOSS = -2000 → a very large margin ensures clear distinction between game outcomes.

constexpr int WIN = 2000;
constexpr int DRAW = 0;
constexpr int LOSS = -2000;
...
std::vector<std::vector<std::pair<int, int>>> winning_states
{
	...
	// Every diagonal -two possible wins only
...
std::pair<int, std::pair<int, int>> minimax_optimization(char board[3][3], char marker, int depth, int alpha, int beta)
{
	...
	for (int i = 0; i < legal_moves.size(); i++)
	{
		...
		if (marker == AI_MARKER)
		{
			...
			if (best_score < score)
			{
				best_score = score - depth * 10;//penalize deeper wins prefer earlier wins
				best_move = curr_move;
		...
		else
		{
			...
			if (best_score > score)
			{
				best_score = score + depth * 10; //Add depth * 10 so that later losses are better than earlier ones (delays losing).
				best_move = curr_move;
